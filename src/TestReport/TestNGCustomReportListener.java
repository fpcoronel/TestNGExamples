	...
	private void resultSummary(ISuite suite, IResultMap tests, String testname, String style, String details) {
		...
		if (tests.getAllResults().size() > 0) {
			...
			for (ITestNGMethod method : getMethodSet(tests, suite)) {
				...
				buff.append(
						"<td><a href=\"#m\"" + m_methodIndex + ">" + qualifiedName(method) + " " + description != null
								&& description.length() > 0
										...
										: "" + "</a>" + null == testInstanceName ? ""
												: "<br>(" + testInstanceName + ")" + "</td>"
														...
														+ formatter.format(calendar.getTime()) + "</td>"
	...
	public void generateSuiteSummaryReport(List<ISuite> suites) {
		...
		for (ISuite suite : suites) {
			...
			for (ISuiteResult r : tests.values()) {
				...
				summaryCell(suite.getParameter("browserType"), true);// Pato
				writer.println("</td>");

				SimpleDateFormat summaryFormat = new SimpleDateFormat("hh:mm:ss");
				summaryCell(summaryFormat.format(overview.getStartDate()), true); // Pato
				writer.println("</td>");

				summaryCell(summaryFormat.format(overview.getEndDate()), true); // Pato
				writer.println("</td>");
